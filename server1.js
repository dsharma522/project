...
app.options('*', cors());
app.use(express.static(__dirname+'/send_res'));
    ...
    let fl=0;
        ...
        {    if(!rst.length)
            {    console.log(rst.insertId);
                ...
                table+='<html><center><img src="G:/project/website1/ymca_header2.jpg "' 
                     ...
                     } //'<html><center><img src="G:/project/website1/ymca_header2.jpg "' 
                     // ymca_header2.jpgymca_header2.jpg   G:\project\ymca_header2.jpg
                   if(fl==0)
                    table = '<html><center><img src="/send_res/ymca_header2.jpg "' 
                    +'alt="/send_res/ymca_header2.jpg" width="1200" height="150">'
        ...
        {    if(!rst.length)
            {
                ...
                table+='<html><center><img src="ymca_header2.jpg "' 
                               +'alt="ymca_header2.jpg" width="1200" height="130">'
                               +'<br><br><h2>sorry ! no record found</h2>';
                         console.log('zero lenth');fl=1;
                     ...
                     {       table += '<tr><td>' + (i + 1) + '</td><td>' + rst[i].internship+ 
                            +'</td><td>' + rst[i].email + '</td><td>'+rst[i].comp+'</td></tr>';
                     }
                   if(fl==0)
                    table = '<html><center><img src="/ymca_header2.jpgymca_header2.jpg "' 
                    +'alt="ymca_header2.jpg" width="1200" height="130">'
                    +'<br><br><table border="1"><tr><th>Sr.</th><th>'
                    +'Name</th><th>Address</th></tr>' + table + '</table>';
               }
...
///// this is always sending the index.html not the loginc .htmml
app.get('/', function (req, res) { console.log('start');
    res.sendFile(__dirname+'/loginc.html');
});
app.get('/loginc.html', function (req, res) { console.log('start');
    res.sendFile(__dirname+'/loginc.html');
});
app.get('/studentdata.html', function (req, res) { console.log('start');
    res.sendFile(__dirname+'/studentdata.html');
});
app.get('/signup.html', function (req, res) { console.log('start');
    res.sendFile(__dirname+'/signup.html');
});/*
app.get('/ymca_header2.jpg', function (req, res) { console.log('start');
    res.sendFile(__dirname+'/ymca_header2.jpg');
});
app.get('/css/hi.css',function(req,res){
   res.sendFile(__dirname+'/css/hi.css');
});
app.get('/signupc.css',function(req,res){
    res.sendFile(__dirname+'/css/hi.css');
 });*/
